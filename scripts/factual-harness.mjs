...
if (drapeau('--cleanup')) {
  ...
  if (r.ok === false) { console.error(`CLEANUP BLOCKED — ${r.reason}`); process.exit(EXIT.BLOCKED) }
...
if (cas === 'manual') {
  ...
  if (!fichier) { console.error('MANUAL_INVALID: --input fichier.json requis'); process.exit(EXIT.INVALID_INPUT) }
  try {
...
if (drapeau('--json')) {
  ...
  process.exit(EXIT[resultat.verdict] ?? EXIT.FAIL)
}
