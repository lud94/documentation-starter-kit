...
import { type ExternalAiPolicy, LOADING, loadExternalAiPolicy } from '../lib/prospector/externalAiPolicy'
...
export default function AskExternalAI({ lead, onSaveNotes }: { lead: Lead; onSaveNotes: (text: string) => Promise<void> }) {
  ...
  // ── POLITIQUE D'EGRESS — TRI-ÉTAT, DÉFAUT REFUS (lot SEC-0d) ──────────────
  //
  // Deux fail-open corrigés ici, et le second était le pire :
  //   1. `.catch(() => setPolicy({ allowed: true, maskPii: false }))` — une
  //      simple erreur réseau AUTORISAIT l'egress ;
  //   2. tant que `policy` valait `null`, la condition de refus
  //      `policy && !policy.allowed` était fausse et l'interface COMPLÈTE
  //      s'affichait : prompt visible, « Ouvrir » et « Copier » actifs, et
  //      `!!policy?.maskPii` valant `false`, le NOM RÉEL de la personne y
  //      figurait. Un fetch lent — ou qui ne revient jamais — laissait donc
  //      l'egress ouvert indéfiniment.
  //
  // Une impossibilité de vérifier la politique n'est pas une autorisation.
  // `granted` n'est vrai que sur un `allowed === true` strict, venant d'une
  // réponse `2xx` bien formée. Tout le reste — chargement, 403, 503, réseau
  // coupé, JSON malformé, `allowed: "yes"` — reste `denied`.
  const [policy, setPolicy] = useState<ExternalAiPolicy>(LOADING)
  useEffect(() => {
    let alive = true
    loadExternalAiPolicy().then((p) => { if (alive) setPolicy(p) })
    return () => { alive = false }
  }, [])

  const granted = policy.state === 'granted'
  // Le prompt est construit avec le masquage MAXIMAL tant que la politique
  // n'accorde rien : même une fuite d'affichage ne pourrait pas porter de nom.
  const list = prompts(lead, granted ? (policy as any).maskPii : true)
  ...
  // Tout ce qui n'est pas une autorisation EXPLICITE rend cette carte inerte :
  // aucun prompt affiché, aucun bouton d'ouverture, rien à copier.
  if (!granted) {
    ...
    return (
      <div className="card p-5">
        <div className="flex items-center gap-2 text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1">
          ...
          {loading ? 'IA externe — vérification' : 'IA externe désactivée'}
        </div>
        <p className="text-xs text-gray-400">{loading
          ? 'Vérification de la politique de cet espace…'
          : "L'envoi de contexte vers une IA externe est désactivé pour cet espace, ou la politique n'a pas pu être vérifiée. Utilise Jarvis (recherche intégrée)."}</p>
      </div>
    )
  }
