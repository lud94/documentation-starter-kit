...
- { "type":"add_company", "company":"...", "withContacts": true|false } → créer le COMPTE dans Prospector (+ dirigeants en contacts si withContacts).
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (token.trim() !== ref.trim()) return res.status(401).json({ error: 'Jeton invalide.' })
  ...
  try {
    if (!confirm) {
      ...
      if (action && (action.type === 'add_company' || action.type === 'add_person')) {
    ...
    if (action?.type === 'add_person') {
      const id = await identifyLead({ name: action.name, url: action.url || url })
      const isPerson = id.kind === 'person'
      const [firstName, ...rest] = String(action.name || '').split(/\s+/)
      const lead: Lead = {
        ...
        score: 0, temperature: 'warm', status: 'froid', stage: 'to_invite', email: null, phone: null,
        linkedinUrl: /linkedin\.com\/in\//i.test(action.url || url) ? (action.url || url) : undefined,
      }
      await upsertLead(lead, 'admin')
      return res.status(200).json({ reply: `${isPerson ? 'Contact' : 'Compte'} « ${action.name} » ajouté à Prospector.`, done: true })
    }
