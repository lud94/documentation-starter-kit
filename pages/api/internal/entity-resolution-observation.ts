...
//
// ── ENTITY_RESOLUTION_OUTCOME_PERSISTENCE_002 ───────────────────────────────
// CHAQUE passage registre abouti persiste EN PLUS un ERX append-only —
// l'entrée durable de la future revue d'entité hors-ligne. Cette écriture est
// BLOQUANTE : un succès runtime sans ERX laisserait la revue reconstruire un
// état périmé ⇒ échec d'ERX = 503 ENTITY_RESOLUTION_OUTCOME_WRITE_FAILED,
// même quand un ERO vient d'être écrit (append-only : JAMAIS de rollback —
// un nouveau passage produira une nouvelle paire valide). Les pannes,
// requêtes invalides et historiques corrompus n'enregistrent RIEN : ce ne
// sont pas des issues. La branche « raison sociale vide » ne produit pas
// d'ERX non plus : AUCUN passage registre n'a eu lieu — c'est un candidat
// malformé, pas une connaissance d'entité.
...
import { recordEntityResolutionOutcome } from '../../../lib/prospector/proactive/entityResolutionOutcome'
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Aucun passage registre : aucune issue à enregistrer.
    ...
    // UN passage registre = UNE horloge serveur. Les branches à ERO utilisent
    // le `retrievedAt` de l'observation (même passage, même instant).
    const instantPassage = new Date().toISOString()
    ...
    if (lookup?.found === true && lookup.resolution === 'resolved') {
      ...
      if (conflit && Array.isArray(lookup.candidates) && lookup.candidates.length > 0) {
        ...
        const issue = await recordEntityResolutionOutcome({
          subjectCandidateId: candidate.id, outcome: 'CONFLICT_REMEDIATION',
          siren: sirenAuto, observationId: o.id, observedAt: o.retrievedAt,
        }, ws)
        // ⚠️ Append-only : l'ERO écrit RESTE — aucun rollback. Le 503 dit la
        // vérité : ce passage n'est pas complet tant que l'issue n'est pas là.
        if (issue.ok === false) return res.status(503).json({ state: 'ENTITY_RESOLUTION_OUTCOME_WRITE_FAILED' })
      ...
      // mais l'ISSUE, elle, est persistée : c'est précisément le fait qui
      // manquait à toute reconstruction hors-ligne (audit 001).
      const issue = await recordEntityResolutionOutcome({
        subjectCandidateId: candidate.id, outcome: 'AUTO_EXACT',
        siren: sirenAuto, observedAt: instantPassage,
      }, ws)
      if (issue.ok === false) return res.status(503).json({ state: 'ENTITY_RESOLUTION_OUTCOME_WRITE_FAILED' })
    ...
    if (!Array.isArray(lookup?.candidates) || lookup.candidates.length === 0) {
      const issue = await recordEntityResolutionOutcome({
        subjectCandidateId: candidate.id, outcome: 'NOT_FOUND', observedAt: instantPassage,
      }, ws)
      if (issue.ok === false) return res.status(503).json({ state: 'ENTITY_RESOLUTION_OUTCOME_WRITE_FAILED' })
    ...
    const issue = await recordEntityResolutionOutcome({
      subjectCandidateId: candidate.id, outcome: 'AMBIGUOUS',
      observationId: o.id, observedAt: o.retrievedAt,
    }, ws)
    if (issue.ok === false) return res.status(503).json({ state: 'ENTITY_RESOLUTION_OUTCOME_WRITE_FAILED' })
    return res.status(200).json({
